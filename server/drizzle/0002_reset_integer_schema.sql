-- Dev reset script: drops the public schema and recreates all app tables using the current integer-based Drizzle schema.
-- Use this only if you are okay losing existing data in the target database.
...
CREATE TYPE "public"."activity_action_type" AS ENUM('CREATE', 'UPDATE', 'DELETE', 'VIEW', 'LOGIN', 'LOGOUT', 'APPROVE', 'REJECT', 'SEND', 'RECEIVE');
CREATE TYPE "public"."activity_entity_type" AS ENUM('USER', 'VENDOR', 'RFQ', 'QUOTATION', 'PURCHASE_ORDER', 'INVOICE', 'CATEGORY');
CREATE TYPE "public"."approval_status" AS ENUM('PENDING', 'APPROVED', 'REJECTED');
CREATE TYPE "public"."invoice_status" AS ENUM('GENERATED', 'SENT', 'VIEWED', 'PAID', 'PARTIALLY_PAID', 'OVERDUE', 'CANCELLED');
CREATE TYPE "public"."po_status" AS ENUM('CREATED', 'SENT', 'ACKNOWLEDGED', 'PARTIALLY_FULFILLED', 'COMPLETED', 'CANCELLED');
CREATE TYPE "public"."quotation_status" AS ENUM('DRAFT', 'SUBMITTED', 'SELECTED', 'REJECTED');
CREATE TYPE "public"."rfq_status" AS ENUM('DRAFT', 'OPEN', 'CLOSED', 'APPROVED', 'REJECTED');
CREATE TYPE "public"."user_role" AS ENUM('ADMIN', 'PROCUREMENT_OFFICER', 'MANAGER', 'VENDOR');
...
ALTER TABLE "rfqs" ADD CONSTRAINT "rfqs_created_by_users_id_fk" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;
...
ALTER TABLE "quotation_items" ADD CONSTRAINT "quotation_items_rfq_item_id_rfq_items_id_fk" FOREIGN KEY ("rfq_item_id") REFERENCES "public"."rfq_items"("id") ON DELETE no action ON UPDATE no action;
ALTER TABLE "quotations" ADD CONSTRAINT "quotations_rfq_id_rfqs_id_fk" FOREIGN KEY ("rfq_id") REFERENCES "public"."rfqs"("id") ON DELETE no action ON UPDATE no action;
ALTER TABLE "quotations" ADD CONSTRAINT "quotations_vendor_id_vendors_id_fk" FOREIGN KEY ("vendor_id") REFERENCES "public"."vendors"("id") ON DELETE no action ON UPDATE no action;
...
ALTER TABLE "approvals" ADD CONSTRAINT "approvals_quotation_id_quotations_id_fk" FOREIGN KEY ("quotation_id") REFERENCES "public"."quotations"("id") ON DELETE cascade ON UPDATE no action;
...
ALTER TABLE "purchase_orders" ADD CONSTRAINT "purchase_orders_rfq_id_rfqs_id_fk" FOREIGN KEY ("rfq_id") REFERENCES "public"."rfqs"("id") ON DELETE no action ON UPDATE no action;
ALTER TABLE "purchase_orders" ADD CONSTRAINT "purchase_orders_quotation_id_quotations_id_fk" FOREIGN KEY ("quotation_id") REFERENCES "public"."quotations"("id") ON DELETE no action ON UPDATE no action;
...
ALTER TABLE "invoices" ADD CONSTRAINT "invoices_po_id_purchase_orders_id_fk" FOREIGN KEY ("po_id") REFERENCES "public"."purchase_orders"("id") ON DELETE no action ON UPDATE no action;
